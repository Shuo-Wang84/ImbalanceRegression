...
# 设置全局参数
NUM_RUNS = 5  # 运行次数
all_bins_values = range(7, 9, 2)
min_mse_results = []
SEED_BASE = 999
iterations = 10  # 自训练迭代次数
# 文件保存路径
output_dir = r"F:\SGIR\SGIR-main\plt画图\MLP\空气质量\自适应分箱"
...
def new_select_high_confidence_samples(model, X_unlabeled, n_perturb=5, top_k=1, stability_thresshold=0.05,
    ...
    model.eval()
    # 随机选择一个较小的背景数据集
    background_indices = np.random.choice(len(X_unlabeled), min(3000, len(X_unlabeled)), replace=False)
