...
# 文件保存路径
output_dir = r"F:\SGIR\SGIR-main\plt画图\XGBoost\空气质量"

def create_xgboost_model():
    """创建XGBoost回归模型"""
    return xgb.XGBRegressor(
        n_estimators=200,
        min_child_weight=3,  # 增加最小权重，防止过拟合
        max_depth=6,
        learning_rate=0.05,
        gamma=0.1,  # 最小分裂损失
        subsample=0.8,
...
def run_experiment(run_seed):
    ...
    # 去除非数值列（Date和Time），并将-200替换为NaN
    data = data.drop(columns=['Date', 'Time'], errors='ignore').replace(-200, np.nan)

    # 只保留缺失值比例小于50%的特征
    valid_columns = data.columns[data.isnull().mean() < 0.5]
