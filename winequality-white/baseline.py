...
def apply_smogn(X, y):
    ...
    y = y.reset_index(drop=True).rename('quality')  # 确保列名正确
    ...
    X_resampled = smogn_data.drop(columns='quality')
    y_resampled = smogn_data['quality']
...
def run_experiment_with_baselines(run_seed):
    ...
    y = data['quality'].rename('quality')  # 标签名改为quality
    ...
    # 保持 y_train 的列名
    y_labeled = pd.Series(y_labeled, name='quality', dtype=np.float32)
