...
# 文件保存路径
output_dir = r"F:\SGIR\SGIR-main\plt画图\MLP\加利福尼亚房价预测\自适应分箱"
...
def new_select_high_confidence_samples(model, X_unlabeled, n_perturb=5, top_k=1, threshold=0.05,
    ...
    # 计算SHAP特征重要性
    # 随机选择一个较小的背景数据集
    background_indices = np.random.choice(len(X_unlabeled), min(500, len(X_unlabeled)), replace=False)
        ...
        for i in range(iterations):
            ...
            for i, (name, importance) in enumerate(zip(feature_names, feature_importance)):
                ...
                if isinstance(importance, np.ndarray):
                    importance_value = importance.item() if importance.size == 1 else importance.mean()
                else:
                    importance_value = importance
                print(f"{i + 1}. {name}: {importance_value:.4f}")
