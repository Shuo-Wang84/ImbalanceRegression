...
def apply_smogn(X, y):
    ...
    y = y.reset_index(drop=True).rename('MedHouseVal')  # 确保列名正确
    ...
    X_resampled = smogn_data.drop(columns='MedHouseVal')
    y_resampled = smogn_data['MedHouseVal']
...
def run_experiment_with_baselines(run_seed):
    ...
    y = data['MedHouseVal'].rename('MedHouseVal')  # 确保列名存在
    ...
    # 保持 y_train 的列名
    y_labeled = pd.Series(y_labeled, name='MedHouseVal', dtype=np.float32)
    ...
    # 新增：分区MSE/MAE
    bin_counts, bins = calculate_bins_sample(y_test, num_bins=16)
