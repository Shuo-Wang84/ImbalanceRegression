...
def apply_smogn(X, y):
    ...
    y = y.reset_index(drop=True).rename('count')  # 确保列名正确
    ...
    X_resampled = smogn_data.drop(columns='count')
    y_resampled = smogn_data['count']
...
def run_experiment_with_baselines(run_seed):
    ...
    # 保持 y_train 的列名
    y_labeled = pd.Series(y_labeled, name='count', dtype=np.float32)
