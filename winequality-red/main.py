...
# 设置全局参数
NUM_RUNS = 5  # 运行次数
all_bins_values = range(6, 8, 2)
min_mse_results = []
SEED_BASE = 999
iterations = 10  # 自训练迭代次数
# 文件保存路径
output_dir = r"F:\SGIR\SGIR-main\plt画图\MLP\红葡萄酒质量预测\自适应分箱"
...
# 新增：多锚点版 Mixup（参数按红酒数据集保守设置）
def label_anchored_mixup_multi(
    X: pd.DataFrame,
    y: pd.Series,
    sampling_counts: pd.Series,
    bins,
    alpha: float = 0.2,            # 红酒推荐：0.2
    method: str = "kmeans",           # 若不稳可切 "kmeans"
    noise_scale: float = 0.03,     # 红酒推荐：0.03
    random_state: int = 42
):
    # ... existing code ...
    # 索引对齐，避免错位
    ...
    anchors_by_bin, bin_info = build_multi_anchors(
        X, y, bins,
        N_min_bin_for_multi=30,    # 红酒：分箱样本不足直接单锚点
        N_min_component=10,        # 红酒：每簇至少 10
        k_max=3,                   # 红酒：最多 3 个锚点
        method=method,
    ...
    for interval, needed in sampling_counts.items():
        ...
        for anchor in anchors:
            ...
            comp_stds.append(X_comp.std().values if len(X_comp) > 1 else np.ones(X.shape[1]) * 1e-6)

        for _ in range(int(needed)):
            a_idx, b_idx = np.random.choice(len(anchors), size=2, replace=False)
            A, B = anchors[a_idx], anchors[b_idx]
...
def run_experiment(run_seed):
    ...
    for bins in all_bins_values:
        ...
        # 划分区间
        bins_sample, bins_edges = kde_adaptive_binning(y_labeled, bins, plot=False)
        ...
        for i in range(iterations):
            ...
            lblsampling_counts, lblbins, max_samples_in_conbined = calculate_kde_sampling_rates(y_combined,
                                                                                            num_bins=all_bins)
            X_augmented, y_augmented = label_anchored_mixup_multi(X_real, y_real, lblsampling_counts, lblbins, alpha=0.2, method="gmm", noise_scale=0.03)
