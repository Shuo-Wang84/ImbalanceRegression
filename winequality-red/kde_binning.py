...
def _resolve_bandwidth(y_values, bandwidth):
    # y_values: 形状 (n, 1)
    ...
    n = len(y_flat)
    # 样本标准差与四分位距
    std = np.std(y_flat, ddof=1)
    q75, q25 = np.percentile(y_flat, [75, 25])
    iqr = max(q75 - q25, 1e-12)

    if bandwidth == 'scott':
        # Scott: h = sigma * n^(-1/5)
        sigma = std if std > 0 else iqr / 1.34
        return float(sigma * n ** (-1.0 / 5.0))
    if bandwidth == 'silverman':
        # Silverman: h = 0.9 * min(sigma, IQR/1.34) * n^(-1/5)
        sigma = std
        return float(0.9 * min(sigma, iqr / 1.34) * n ** (-1.0 / 5.0))
    return 0.4  # 对红酒数据较稳妥的数值

def kde_adaptive_binning(y, num_bins=50, bandwidth='silverman', min_density_threshold=0.2, plot=True):
    """
    使用核密度估计(KDE)进行自适应分箱
    ...
    bw = _resolve_bandwidth(y_values, bandwidth)
...
def calculate_kde_sampling_rates(y, num_bins=50, bandwidth='silverman', min_density_threshold=0.2, plot=False):
    """
