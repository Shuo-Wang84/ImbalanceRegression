...
# 文件保存路径
output_dir = r"F:\SGIR\SGIR-main\plt画图\XGBoost\红酒品质"

def create_xgboost_model():
    """创建XGBoost回归模型"""
    return xgb.XGBRegressor(
        n_estimators=150,
        min_child_weight=5,  # 增加最小权重，防止过拟合
        max_depth=4,
        learning_rate=0.08,
        subsample=0.85,
        colsample_bytree=0.85,  # 列采样
        colsample_bylevel=0.85,  # 每层列采样
        # 正则化参数 - 对小数据集很重要
        reg_alpha=0.3,  # L1正则化，增强稀疏性
        reg_lambda=1.5,  # L2正则化，防止过拟合
        random_state=42,
        n_jobs=-1
    )
...
def train_xgboost_model(model, X_train, y_train, use_fixed_seed=False):
    """训练XGBoost模型"""
    if use_fixed_seed:
        model.set_params(random_state=SEED_BASE)
...
def divide_regions(sample_counts):
    """
...
def new_select_high_confidence_samples(model, X_unlabeled, n_perturb=5, top_k=1, threshold=0.05,
    ...
    shap_values = explainer.shap_values(X_unlabeled.values)
    ...
    # 计算每组扰动下的预测
    preds = []
    for perturbed_data in perturbed_samples_list:
        pred = model.predict(perturbed_data)  # XGBoost预测
        preds.append(pred)
...
def run_experiment(run_seed):
    ...
    y = data['quality']
    X_full_train, X_test, y_full_train, y_test = train_test_split(X, y, test_size=0.2, random_state=run_seed)
    ...
    for bins in all_bins_values:
        ...
        base_model = create_xgboost_model()
        pseudo_model = create_xgboost_model()
        augment_model = create_xgboost_model()
        model = create_xgboost_model()

        # model.fit(X_labeled, y_labeled)
        train_xgboost_model(model, X_labeled, y_labeled)

        # base_model.fit(X_labeled, y_labeled)
        train_xgboost_model(base_model, X_labeled, y_labeled)

        # y_test_pred = base_model.predict(X_test)
        y_test_pred = predict_xgboost(base_model, X_test)
        ...
        train_data_region_map = divide_regions(bins_sample)
        ...
        test_region_map = divide_regions(test_bins_sample)
        ...
        for i in range(iterations):
            # 使用模型预测未标记数据的标签
            X_unlbl_pred = predict_xgboost(model, X_unlabeled)
            ...
            y_conf = pd.Series(
                predict_xgboost(model, X_conf),  # 使用 .values 转换为 ndarray，直接传入 DataFrame（模型已记录特征名称）
                index=X_conf.index
            )
            ...
            # 计算使用未标记数据的均方误差（MSE）
            train_xgboost_model(pseudo_model, pd.concat([X_labeled, X_sampled]), pd.concat([y_labeled, y_sampled]))

            # pseudo_model.fit(pd.concat([X_labeled, X_sampled]), pd.concat([y_labeled, y_sampled]))
            y_useunlabel_test_pred = predict_xgboost(pseudo_model, X_test)
            ...
            y_augmented = pd.Series(y_augmented)
            train_xgboost_model(augment_model, pd.concat([X_labeled, X_augmented], ignore_index=True), pd.concat([y_labeled, y_augmented], ignore_index=True))
            ...
            y_usemixupdate_test_pred = predict_xgboost(augment_model, X_test)
            ...
            train_xgboost_model(model, X_final, y_final)

            y_useall_test_pred = predict_xgboost(model, X_test)
