...
def apply_smogn(X, y):
    ...
    y = y.reset_index(drop=True).rename('target')  # 确保列名正确
    ...
    X_resampled = smogn_data.drop(columns='target')
    y_resampled = smogn_data['target']
...
def run_experiment_with_baselines(run_seed):
    ...
    # 保持 y_train 的列名
    y_labeled = pd.Series(y_labeled, name='target', dtype=np.float32)
